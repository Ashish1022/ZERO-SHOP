...
export const AdminCategoriesView = () => {
  ...
  const activeCount = flattenedCategories.filter(c => c.status === 'active').length;
  const featuredCount = flattenedCategories.filter(c => c.featured).length;
  const totalSubcategories = flattenedCategories.reduce((sum, c) => sum + c.subcategoriesCount, 0);
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                {flattenedCategories.filter(c => c.status === 'inactive').length}
              </span>
            </div>
